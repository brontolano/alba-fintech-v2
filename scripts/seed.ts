...
async function main() {
  ...
  for (const unitData of units) {
    ...
    await prisma.unitSetting.upsert({
      ...
      create: {
        ...
        autoApproval: false,
        requiresApproval: unitData.name === "KPAK" ? false : true,
      },
    });
  }
  ...
  // Dummy transactions
  const staffKantin = await prisma.user.findUnique({
    where: { email: "staff.kantin@alba.local" },
  });
  const mgrKoperasi = await prisma.user.findUnique({
    where: { email: "manager.koperasi@alba.local" },
  });
  const mgrKpk = await prisma.user.findUnique({
    where: { email: "manager.kpk@alba.local" },
  });
  const incomeSales = await prisma.financialCategory.findUnique({
    where: { code: "INC-PEN" },
  });
  const incomeAdmin = await prisma.financialCategory.findUnique({
    where: { code: "INC-ADM" },
  });
  const incomeSavings = await prisma.financialCategory.findUnique({
    where: { code: "INC-TAB" },
  });
  const expensePurch = await prisma.financialCategory.findUnique({
    where: { code: "EXP-BEL" },
  });
  const expenseSalary = await prisma.financialCategory.findUnique({
    where: { code: "EXP-GAJ" },
  });

  // Idempotency guard — remove prior dummy rows before re-seeding
  const existingDummy = await prisma.transaction.findMany({
  ...
  const txns = [
    ...
    {
      unitCode: "KOP-01",
      userId: mgrKoperasi!.id,
      categoryId: incomeSales!.id,
      type: "INCOME",
      amount: 750000,
      desc: "[dummy] Penjualan buku/cangkang buku",
    },
    {
      unitCode: "KOP-01",
      userId: mgrKoperasi!.id,
      categoryId: expensePurch!.id,
      type: "EXPENSE",
      amount: 300000,
      desc: "[dummy] Pembelian buku baku dari penerbit",
    },
    // Kantin Umi — penjualan & pengeluaran
    {
      ...
      type: "INCOME",
      amount: 420000,
      desc: "[dummy] Penjualan menu kantin (nasi goreng/mie)",
    },
    {
      unitCode: "KNT-01",
      userId: staffKantin!.id,
  ...
  for (const t of txns) {
    ...
    console.log(
      `💸 Created transaction: ${t.desc.substring(t.desc.indexOf("]") + 2)} at ${unit!.name}`,
    );
  }

  // Catatan keuangan pimpinan (revenue summary)
  const pimpinan = await prisma.user.findUnique({
    where: { email: "pimpinan@alba.local" },
  });
  if (pimpinan) {
    await prisma.financialNote.upsert({
      ...
      create: {
        ...
        description:
          "[dummy] Total pemasukan harian gabungan dari seluruh unit",
        amount: 2435000,
        type: "INCOME",
        date: new Date(),
