/**
 * Run migration directly against production database
 * Usage: node scripts/run-migration-production.mjs
 */
...
async function addColumnIfNotExists(table, column, definition) {
  ...
  if (!exists) {
    console.log(`  ⚠️  Adding ${table}.${column}...`);
...
async function addIndexIfNotExists(table, indexName, columns) {
  try {
    ...
    if (indexes.length === 0) {
      console.log(`  ⚠️  Adding index ${indexName}...`);
...
try {
  ...
  if (await columnExists('transactions', 'date') && await columnExists('transactions', 'transaction_date')) {
    ...
    if (rows[0].c > 0) {
      console.log(`  ⚠️  Copying data from date to transaction_date...`);
  ...
  console.log('📋 Summary:');
  console.log('   - All 16 tables from schema.prisma checked and migrated');
  console.log('   - All missing columns added');
  console.log('   - All foreign key indexes added');
