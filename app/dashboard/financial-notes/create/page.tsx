...
export default function CreateFinancialNotePage() {
  return (
    ...
    >
      <CreateFinancialNoteContent />
    </Suspense>
  );
}

function CreateFinancialNoteContent() {
  ...
  // Mode edit: muat data catatan yang ada
  useEffect(() => {
    if (!editId) return;
    const loadNote = async () => {
      try {
        const res = await fetch(`/api/financial-notes/${editId}`);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetch(
        isEditMode ? `/api/financial-notes/${editId}` : "/api/financial-notes",
        {
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold tracking-tight text-foreground">
              {isEditMode ? "Edit Catatan Keuangan" : "Buat Catatan Keuangan"}
            </h1>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="flex justify-end gap-3 border-t border-border pt-4">
            ...
            <button
              type="submit"
              disabled={
                submitting || !form.amount || !form.title || !form.unitId
              }
