...
export async function PUT(
  ...
  try {
    ...
    if (role !== "SUPERADMIN") {
      return NextResponse.json(
        { error: "Hanya SUPERADMIN yang dapat mengedit pengguna" },
        { status: 403 },
      );
    }
    ...
    return NextResponse.json({
      data: {
        ...updated,
        unit: updated.units,
        lembaga: updated.lembagas,
        units: updated.units,
        lembagas: updated.lembagas,
      },
    });
  } catch (err: any) {
    if (err.code === "P2002") {
...
export async function DELETE(
  ...
  try {
    ...
    return NextResponse.json({
      message: `Pengguna "${existing.name || existing.email}" berhasil dihapus`,
    });
  } catch (err: any) {
    if (err.code === "P2002") {
      return NextResponse.json(
