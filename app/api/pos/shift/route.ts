...
// Perhitungan hari memakai WIB (UTC+7) agar konsisten dgn modul lain.
...
function guardRetail(session: any) {
  ...
  if (!["SUPERADMIN", "PIMPINAN", "MANAGER", "STAFF"].includes(role))
    return false;
  // SUPERADMIN/PIMPINAN boleh memantau; MANAGER/STAFF wajib unit retail.
...
/** GET: status absensi saya hari ini + kru unit yang bertugas + log 14 hari */
...
/** POST: check-in / check-out shift kasir */
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json({ error: "Gagal mencatat shift kasir" }, { status: 500 });
  }
}